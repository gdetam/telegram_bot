...
from keyboards.books_keyboards import create_books_keyboard_by_reader
...
async def send_reader_books_by_item_id(
                                       call: CallbackQuery,
                                       item_id: int,
                                       current_page: int,
                                       column_name: Column):
    """Submit books and item_id."""
    books_list = get_genre_and_reader_books_by_item_id(Readers, item_id,
                                                       current_page)
    # keyboard book
    count_page = get_count_books_pages_by_item_id(item_id, column_name)
    book_keyboard = create_books_keyboard_by_reader(books_list, current_page,
                                                    count_page, item_id)
